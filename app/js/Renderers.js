...
//EVERYTHING IN HERE SHOULD HAVE IT'S OWN RENDERER!!
    ...
    // draw xy coordinates
    drawCoordinatePattern(dist) {
        this.ctx.fillStyle = '#ffffff';
        for (let x = 0; x < this.canvas.width; x += dist) {
            for (let y = 0; y < this.canvas.height; y += dist) {
    ...
    // draw point
    drawPoint(x, y, r, color = '#ff0000', label = '', labelColor = '#000', labelFont = '5px Monospace') {
    ...
    // draw string
    drawString(string, x, y, color = '#000', font = '20px Monospace') {
        this.ctx.fillStyle = color;
        this.ctx.font = font;
    ...
    drawRectBorder(x, y, width, height, color = '#000', lineWidth = 1) {
...
const FramerateRenderer = class FramerateRenderer extends BaseRenderer {
    constructor(canvas) {
        super(canvas);
    }

    //draw frame rate
    drawFps(x, y, color = '#000', font = '20px Monospace') {
        // get fps
        let fps = Cycle.getInstance().fps;
        ...
        // draw fps
        this.drawString(fps, x, y, color, font);
    }
}
